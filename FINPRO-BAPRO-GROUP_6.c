...
int main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        if (choice == 1) {
            ...
            printf("1. BMI\n");
            printf("2. Blood Pressure\n");
            printf("3. Glucose Level\n");
            printf("4. Cholesterol\n");
            printf("5. Sleep\n");
            scanf("%d", &choice);
...
void displayFullReport(const HealthData *person) {
    ...
    if (cholesterolStatus == CRITICAL) {
        printf("Critical Condition (contact a doctor immedietly)\n");
    } else if (cholesterolStatus == AT_RISK) {
        printf("At Risk\n");
    } else {
        printf("Healthy\n");
    }
    ...
    if (person->dailywaterintake < 2.0) {
        printf("- Increase daily water intake to maintain hydration.\n");
    }
}